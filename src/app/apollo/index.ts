...
import {
  ...
  TOKEN_NOT_FOUND, UNAUTHORIZED, CONFLICT_EXCEPTION, INVALID_AUTH_TOKEN, UNAUTHENTICATED
} from "../constants";
...
const httpLink = new HttpLink({
  uri: `${import.meta.env.VITE_REACT_APP_API_BASE_URL}/graphql`
});
...
const errorLink = onError(({ graphQLErrors, networkError }) => {
  if (graphQLErrors) {
    ...
    if (message === UNAUTHORIZED || message === TOKEN_INVALID || message === INVALID_AUTH_TOKEN) handleLogout();
  }
